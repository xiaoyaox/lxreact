...
export function getConfig(success, error) {
    ...
    function toFirstUpperCase (key){
      return key.replace(/\b(\w)|\s(\w)/g, function(m){
    ...
    Client.getConfig(
        (data) => {
            ...
            Object.keys(data).forEach((key) => {
              const upKey = toFirstUpperCase(key);
              result[upKey] = data[key];
              if (result[upKey] && typeof result[upKey] == 'object') {
                const subRes = {};
                Object.keys(result[upKey]).forEach((subKey) => {
                  const upSubKey = toFirstUpperCase(subKey);
                  subRes[upSubKey] = result[upKey][subKey];
                });
                result[upKey] = subRes;
              }
            });
